...
class R2dbcPostgresIntegrationTest {
    ...
    fun `can execute simple query`() =
        runBlocking {
            val result =
                databaseClient.sql("SELECT 1::integer as value")
    ...
    @Test
    fun `can create and query temporary table`() =
        runBlocking {
        ...
        """,
            ...
            // Query the row
            val name =
                databaseClient.sql("SELECT name FROM test_table WHERE id = 1")
    ...
    @Test
    fun `connection pool is configured`() =
        runBlocking {
            // Execute multiple queries to verify pooling works
            repeat(10) { index ->
                val result =
                    databaseClient.sql("SELECT $index::integer as value")
                        .map { row, _ -> row.get("value", Integer::class.java) }
                        .first()
                        .awaitFirst()
